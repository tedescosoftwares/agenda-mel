...
const RPC = {
  ...
  pdv_repasse: ({ de, ate }) => {
    ...
    const por = ps.map((p) => { const meus = itens.filter((i) => i.professional_id === p.id); const cota = meus.find((i) => i.cota_pct != null); const liq = meus.reduce((t, i) => t + i.liquido_cents, 0); const rep = cota ? meus.reduce((t, i) => t + (i.repasse_cents ?? 0), 0) : null; return { professional_id: p.id, nome: p.name, comandas: new Set(meus.map((i) => i.comanda_id)).size, itens: meus.length, bruto_cents: meus.reduce((t, i) => t + i.valor_cents, 0), desconto_cents: meus.reduce((t, i) => t + i.desconto_cents, 0), liquido_cents: liq, contrato: cota ? { parceria_id: 'pc-' + p.id, cota_pct: cota.cota_pct, base_calculo: cota.cota_pct === 50 ? 'bruto' : 'liquido', periodicidade: 'semanal' } : null, repasse_cents: rep, casa_cents: rep == null ? null : liq - rep } }).filter((x) => x.itens > 0).sort((a, b) => b.liquido_cents - a.liquido_cents)
    return { de, ate, total: { bruto_cents: itens.reduce((t, i) => t + i.valor_cents, 0), desconto_cents: itens.reduce((t, i) => t + i.desconto_cents, 0), liquido_cents: itens.reduce((t, i) => t + i.liquido_cents, 0), repasse_cents: itens.reduce((t, i) => t + (i.repasse_cents ?? 0), 0), comandas: new Set(itens.map((i) => i.comanda_id)).size, itens: itens.length }, por_profissional: por, itens }
  },
