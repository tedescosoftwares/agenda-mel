...
const TABELAS = {
  pagamentos: [
    ...
    { id: 'pg0', appointment_id: 'ap3', client_id: 'c1', salon_id: SALAO, status: 'pago', valor_cents: 3500, total_cents: 3500, sinal_pct: 100, pago_em: mais(-13) + 'T18:02:00', criado_em: mais(-13), cobranca_id: 'pay_demo_2m9q1', termos_aceitos_em: mais(-13) + 'T18:00:00', appointments: { service_name: 'Manicure', date: mais(-12), start_time: '09:00:00', profiles: { full_name: 'Juliana Silva' } } },
  ...
  vinculos: [{ id: 'vc1', client_id: 'c1', salon_id: SALAO, saiu_em: null, trazida_por: 'pr1' }],
...
const RPC = {
  ...
  pagamento_do_salao: () => ({ modo: 'opcional', sinal_pct: 50, politica: 'moderada', estorno_horas: 12, credito_dias: 30, carencia_min: 60 }),
  meu_credito: () => null,
  regras_do_agendamento: () => ({ politica: 'moderada', horas: 12, credito_dias: 30, limite: mais(8) + 'T22:30:00', dentro_do_prazo: true, pago_cents: 3000, taxa_cents: 199, volta_cents: 2801, credito_ate: null, credito_ate_se_cancelar: mais(39), salao: 'Studio Mel', salon_id: SALAO }),
  financeiro_do_salao: () => ({ mes: mais(0).slice(0, 7), recebido_cents: 184000, recebido_anterior_cents: 152000, taxas_cents: 5771, mimo_cents: 0, liquido_cents: 178229, pagamentos: 29, clientes: 21, devolvido_cents: 9801, a_devolver_cents: 0, devolucoes_paradas: 0, retido_cents: 3500, creditos_cents: 7000, creditos: 2, a_receber_cents: 96000, aguardando: 1, saldo_cents: 141230, saldo_em: new Date().toISOString(),
    por_profissional: [{ id: 'pr1', nome: 'Ana Oliveira', quantos: 14, recebido_cents: 92000, liquido_cents: 89214 }, { id: 'pr2', nome: 'Camila Rocha', quantos: 9, recebido_cents: 58000, liquido_cents: 56209 }, { id: 'pr3', nome: 'Fernanda Lima', quantos: 6, recebido_cents: 34000, liquido_cents: 32806 }],
    por_dia: Array.from({ length: 12 }, (_, i) => ({ dia: mais(-(11 - i)), recebido_cents: [9000, 14000, 0, 21000, 12000, 18000, 7000, 24000, 16000, 20000, 11000, 32000][i] })),
    movimentos: [
      { id: 'm1', status: 'pago', valor_cents: 3000, total_cents: 6000, sinal_pct: 50, liquido_cents: 2801, mimo_cents: 0, quando: mais(-1), cliente: 'Juliana Silva', servico: 'Escova', dia: mais(9), hora: '10:30:00', profissional: 'Ana Oliveira' },
      { id: 'm2', status: 'estornado', valor_cents: 4500, total_cents: 4500, sinal_pct: 100, liquido_cents: 4301, estorno_cents: 4301, quando: mais(-2), cliente: 'Carla Mendes', servico: 'Manicure', dia: mais(3), hora: '14:00:00', profissional: 'Camila Rocha' },
      { id: 'm3', status: 'credito', valor_cents: 3500, total_cents: 7000, sinal_pct: 50, liquido_cents: 3301, credito_ate: mais(28), quando: mais(-3), cliente: 'Mariana Souza', servico: 'Spa dos pés', dia: mais(-1), hora: '16:00:00', profissional: 'Fernanda Lima' },
      { id: 'm4', status: 'retido', valor_cents: 3500, total_cents: 7000, sinal_pct: 50, liquido_cents: 3301, quando: mais(-6), cliente: 'Beatriz Souza', servico: 'Corte', dia: mais(-5), hora: '11:00:00', profissional: 'Ana Oliveira' },
    ] }),
  ...
  pagina_do_salao: () => ({
    ...
    servicos: servicos.filter((s) => s.active).map((s) => ({ ...s, quem: vinculos.filter((v) => v.service_id === s.id).map((v) => ({ id: v.professional_id, nome: profissionais.find((p) => p.id === v.professional_id)?.name ?? '', foto: profissionais.find((p) => p.id === v.professional_id)?.photo_url ?? null })) })),
    capas: { ct2: [PROMO_IMG('#FF2D7A', '#AA4CFF', ''), PROMO_IMG('#AA4CFF', '#FF7BAA', '')] },
    preferida: 'pr1',
  ...
  telefone_disponivel: ({ fone }) => (String(fone).replace(/\D/g, '').endsWith('0115') ? { disponivel: false, em_uso: true, motivo: 'Esse WhatsApp já está em uso e não dá pra cadastrar de novo. Se o número é seu, fale com o suporte.' } : { disponivel: true }),
  ...
  pdv_dia: ({ dia } = {}) => {
    ...
    const linhas = [['ap1', 'c1', 'pr1', 'sv2', '09:00:00', '10:30:00', 'concluido', 8500, 0], ['apx1', 'c2', 'pr2', 'sv7', '10:30:00', '11:15:00', 'confirmado', 6000, 3000], ['apx2', 'c3', 'pr1', 'sv1', '11:00:00', '11:45:00', 'confirmado', 3500, 0], ['apx3', 'c4', 'pr3', 'sv4', '14:00:00', '15:00:00', 'pendente', 6500, 0], ['apx4', 'c5', 'pr2', 'sv3', '15:30:00', '16:30:00', 'confirmado', 12000, 0], ['apx5', 'c2', 'pr1', 'sv1', '11:15:00', '12:00:00', 'confirmado', 3500, 0]]
    return {
      dia: hoje,
      agenda: linhas.map(([id, cid, pid, sid, ini, fim, status, preco, pago], i) => { const s = servicos.find((x) => x.id === sid); const p = profissionais.find((x) => x.id === pid); const cl = clientes.find((x) => x.id === cid); const pref = ['pr1', 'pr1', null, 'pr3', 'pr2', 'pr1'][i]; return { id, start_time: ini, end_time: fim, status, client_id: cid, cliente: cl?.full_name ?? 'Cliente', professional_id: pid, profissional: p?.name, servico: s?.name, price_cents: preco, pago_cents: pago, itens: s ? [{ service_id: s.id, nome: s.name, preco_cents: preco, duracao: s.duration_minutes, qtd: 1 }] : [], comanda_id: id === 'ap1' ? 'cm1' : null, avaliacao: id === 'ap1' ? { nota: 5, comentario: 'Amei, saiu perfeita! A Ana é um amor.', em: hoje + 'T11:10:00' } : null, atendimentos: [9, 3, 0, 6, 1, 3][i], ultima_visita: [mais(-12), mais(-20), null, mais(-40), mais(-90), mais(-20)][i], faltas: [0, 0, 0, 1, 0, 0][i], preferida_id: pref, preferida: pref ? profissionais.find((x) => x.id === pref)?.name : null } }).sort((x, y) => x.start_time.localeCompare(y.start_time)),
      ...
      caixa: { total_cents: 8500, balcao_cents: 8500, app_cents: 0, por_forma: [{ forma: 'pix', valor_cents: 8500 }], por_profissional: [{ professional_id: 'pr1', nome: 'Ana Oliveira', valor_cents: 8500, comandas: 1 }] },
    }
  },
  ...
  salao_cota_padrao: ({ pct, base }) => ({ cota_pct: pct, base: base ?? 'liquido' }),
  projecao_semanal: ({ inicio, prof }) => {
    ...
    const hoje = mais(0)
    const fator = prof ? 0.3 : 1
    const linhas = dias.map((dia, i) => { const n = Math.round([6, 5, 7, 4, 8, 9, 0][i] * fator); const preco = n * 7500; const passado = dia < hoje; const ehHoje = dia === hoje; const conc = passado ? n : ehHoje ? Math.floor(n / 2) : 0; const pend = passado ? 0 : Math.floor(n / 4); const conf = n - conc - pend; return { dia, horarios: n, confirmados: conf, pendentes: pend, concluidos: conc, faltas: passado && n > 5 ? 1 : 0, previsto_cents: preco, confirmado_cents: conf * 7500, pendente_cents: pend * 7500, realizado_cents: conc * 7500, sinal_cents: (conf + pend) * 2500, perdido_cents: passado && n > 5 ? 6000 : 0, minutos_marcados: n * 55, minutos_abertos: i === 6 ? 0 : 4 * 540 * fator } })
    ...
    return { inicio: seg, fim: dias[6], hoje, dias: linhas,
      padrao: { cota_pct: 50, base: 'liquido' },
      total: { horarios: soma('horarios'), confirmados: soma('confirmados'), pendentes: soma('pendentes'), concluidos: soma('concluidos'), faltas: soma('faltas'), previsto_cents: soma('previsto_cents'), confirmado_cents: soma('confirmado_cents'), pendente_cents: soma('pendente_cents'), realizado_cents: soma('realizado_cents'), sinal_cents: soma('sinal_cents'), perdido_cents: soma('perdido_cents'), equipe_cents: Math.round(soma('previsto_cents') * 0.5), casa_cents: Math.round(soma('previsto_cents') * 0.5), ticket_medio_cents: soma('horarios') ? Math.round(soma('previsto_cents') / soma('horarios')) : 0, minutos_marcados: soma('minutos_marcados'), minutos_abertos: soma('minutos_abertos') },
      por_profissional: (prof ? ps.filter((p) => p.id === prof) : ps).map((p, i) => ({ professional_id: p.id, nome: p.name, horarios: [12, 10, 9, 8][i], pendentes: [2, 1, 1, 0][i], previsto_cents: [90000, 75000, 67500, 60000][i], realizado_cents: [37500, 30000, 22500, 15000][i], sinal_cents: [15000, 12500, 10000, 7500][i], cota_pct: [40, 50, 50, 50][i], contrato: i === 0, equipe_cents: [36000, 37500, 33750, 30000][i], casa_cents: [54000, 37500, 33750, 30000][i], minutos_marcados: [660, 550, 495, 440][i], minutos_abertos: [3240, 3240, 2700, 2160][i] })),
      semanas: [-4, -3, -2, -1, 0].map((k) => { const d = new Date(seg + 'T12:00:00'); d.setDate(d.getDate() + k * 7); const ini = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`; const base = [241000, 258500, 231000, 268500, soma('previsto_cents')][k + 4]; return { inicio: ini, previsto_cents: k === 0 ? base : base + 12000, realizado_cents: k === 0 ? soma('realizado_cents') : base, horarios: k === 0 ? soma('horarios') : 30 + k } }),
  ...
  pdv_repasse: ({ de, ate }) => {
    const ps = profissionais.filter((p) => p.active).slice(0, 3)
    const itens = [
      ['i1', mais(0), 'Juliana Silva', ps[0], 'Coloração', 22000, 1000, 40, 'liquido'], ['i2', mais(0), 'Juliana Silva', ps[0], 'Escova Simples', 7000, 0, 40, 'liquido'], ['i3', mais(0), 'Juliana Silva', ps[1], 'Pedicure', 4000, 0, null, null],
      ['i4', mais(-1), 'Carla Mendes', ps[1], 'Manicure', 3500, 0, null, null], ['i5', mais(-1), 'Carla Mendes', ps[0], 'Corte feminino', 9000, 500, 40, 'liquido'], ['i6', mais(-2), 'Mariana Souza', ps[2], 'Spa dos pés', 6500, 0, 50, 'bruto'],
      ['i7', mais(-3), 'Beatriz Costa', ps[2], 'Massagem relaxante', 13000, 0, 50, 'bruto'], ['i8', mais(-4), 'Juliana Prado', ps[0], 'Botox', 10000, 0, 40, 'liquido'],
    ].filter(([, d]) => d >= de && d <= ate).map(([id, dia, cliente, p, nome, valor, desc, cota, base]) => ({ id, dia, fechada_em: dia + 'T15:20:00', comanda_id: 'cm-' + id, cliente, professional_id: p?.id, profissional: p?.name, nome, qtd: 1, valor_cents: valor, desconto_cents: desc, liquido_cents: valor - desc, cota_pct: cota, repasse_cents: cota == null ? null : Math.round((base === 'liquido' ? valor - desc : valor) * cota / 100) }))
    const por = ps.map((p) => { const meus = itens.filter((i) => i.professional_id === p.id).map((i) => (i.cota_pct == null ? { ...i, cota_pct: 50, repasse_cents: Math.round(i.liquido_cents / 2) } : i)); const cota = meus.find((i) => i.cota_pct !== 50); const liq = meus.reduce((t, i) => t + i.liquido_cents, 0); const rep = meus.reduce((t, i) => t + (i.repasse_cents ?? 0), 0); return { professional_id: p.id, nome: p.name, comandas: new Set(meus.map((i) => i.comanda_id)).size, itens: meus.length, bruto_cents: meus.reduce((t, i) => t + i.valor_cents, 0), desconto_cents: meus.reduce((t, i) => t + i.desconto_cents, 0), liquido_cents: liq, contrato: cota ? { origem: 'contrato', parceria_id: 'pc-' + p.id, cota_pct: cota.cota_pct, base_calculo: 'liquido', periodicidade: 'semanal' } : { origem: 'casa', parceria_id: null, cota_pct: 50, base_calculo: 'liquido', periodicidade: null }, repasse_cents: rep, casa_cents: liq - rep } }).filter((x) => x.itens > 0).sort((a, b) => b.liquido_cents - a.liquido_cents)
    return { de, ate, padrao: { cota_pct: 50, base: 'liquido' }, total: { bruto_cents: itens.reduce((t, i) => t + i.valor_cents, 0), desconto_cents: itens.reduce((t, i) => t + i.desconto_cents, 0), liquido_cents: itens.reduce((t, i) => t + i.liquido_cents, 0), repasse_cents: itens.reduce((t, i) => t + (i.repasse_cents ?? 0), 0), comandas: new Set(itens.map((i) => i.comanda_id)).size, itens: itens.length }, por_profissional: por, itens }
  },
  ...
  adicionar_servico_ao_horario: () => ({ ok: true, esticou: true }),
  ...
  pdv_historico: () => [
    { id: 'h1', dia: mais(-12), hora: '09:00:00', status: 'concluido', servico: 'Manicure', profissional: 'Ana Oliveira', price_cents: 3500, pago_cents: 0, itens: [{ nome: 'Manicure', preco_cents: 3500 }], comanda: { total_cents: 3500, pagamentos: [{ forma: 'pix', valor_cents: 3500 }] }, avaliacao: 5, comentario: 'Unhas impecáveis, como sempre.' },
