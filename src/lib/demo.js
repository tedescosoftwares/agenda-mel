...
const RPC = {
  ...
  pdv_dia: ({ dia } = {}) => {
    ...
    return {
      dia: hoje,
      agenda: linhas.map(([id, cid, pid, sid, ini, fim, status, preco, pago], i) => { const s = servicos.find((x) => x.id === sid); const p = profissionais.find((x) => x.id === pid); const cl = clientes.find((x) => x.id === cid); const pref = ['pr1', 'pr1', null, 'pr3', 'pr2'][i]; return { id, start_time: ini, end_time: fim, status, client_id: cid, cliente: cl?.full_name ?? 'Cliente', professional_id: pid, profissional: p?.name, servico: s?.name, price_cents: preco, pago_cents: pago, itens: s ? [{ service_id: s.id, nome: s.name, preco_cents: preco, duracao: s.duration_minutes, qtd: 1 }] : [], comanda_id: id === 'ap1' ? 'cm1' : null, atendimentos: [9, 3, 0, 6, 1][i], ultima_visita: [mais(-12), mais(-20), null, mais(-40), mais(-90)][i], faltas: [0, 0, 0, 1, 0][i], preferida_id: pref, preferida: pref ? profissionais.find((x) => x.id === pref)?.name : null } }),
