...
export default function ClienteAgendamento() {
  ...
  // depois que o salão fechou (concluído ou com comanda), não falta mais nada: o sinal vira só um detalhe de como pagou
  const fechado = a.status === 'concluido' || Boolean(comandaId)
        ...
        <div className={'card pag-estado ' + (a.status === 'aguardando_pagamento' ? 'aguardando' : pagamento?.status ?? 'pago')}>
          ...
          <span>
            {a.status === 'aguardando_pagamento' ? <><strong>Aguardando o PIX</strong><span className="muted">A vaga fica guardada por 15 minutos.</span></>
              ...
              : a.pago_cents > 0 && fechado ? <><strong>Atendimento pago</strong><span className="muted">{a.pago_cents < (a.price_cents ?? 0) ? `Sinal de ${formatCents(a.pago_cents)} pelo app e o restante acertado no salão.` : `${formatCents(a.pago_cents)} pagos pelo app.`}</span></>
