...
# migração se aplica em ordem: tudo abaixo da MAIOR anotada já está no
# banco (a anotação nasceu na 040; o que veio antes entrou pelo setup
# antigo). Conta como feito e fica anotado, para não perguntar de novo.
# Rodar migração velha em banco novo quebra: a 004 cria uma regra que a
# 016 substituiu.
MAIOR=$(printf '%s\n' "$APLICADAS" | grep -E '^[0-9]{3}_' | sort | tail -1 | cut -c1-3)
if [ -n "$MAIOR" ]; then
  for f in $(ls -1 [0-9][0-9][0-9]_*.sql | sort | awk -F_ -v m="$MAIOR" '$1+0 < m+0'); do
