-- Ensaio da conclusão automática (075). Roda no banco de teste e desfaz.
begin;
do $$
...
begin
  ...
  -- ontem, confirmado: conclui
  insert into public.appointments (client_id, professional_id, service_id, date, start_time, end_time, salon_id, status)
  values (cli, prof.id, svc.id, current_date - 1, '10:00', '11:00', prof.salon_id, 'confirmado') returning id into velho;
  ...
  -- ontem, confirmado, mas com pedido de troca aberto: espera
  insert into public.appointments (client_id, professional_id, service_id, date, start_time, end_time, salon_id, status)
  values (cli, prof.id, svc.id, current_date - 1, '14:00', '15:00', prof.salon_id, 'confirmado') returning id into comtroca;
