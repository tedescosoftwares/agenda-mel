...
begin
  ...
  perform set_config('request.jwt.claim.sub', dona::text, false);
  set role authenticated;
  r := public.onboarding_salvar(sal, jsonb_build_object('documento_tipo', 'cnpj', 'cnpj', '69.089.327/0001-88', 'razao_social', 'Mimo Ltda', 'nome_fantasia', 'MIMO', 'endereco_fiscal', jsonb_build_object('address', 'Rua Pais Leme, 215', 'city', 'São Paulo', 'uf', 'SP', 'cep', '05424150'), 'endereco_igual', false, 'contatos', jsonb_build_array(jsonb_build_object('tipo', 'telefone', 'valor', '1134567890', 'escopo', 'cadastro'), jsonb_build_object('tipo', 'email', 'valor', 'financeiro@ensaio.com', 'escopo', 'salao')), 'fotos', jsonb_build_array('https://x/a.jpg', 'https://x/b.jpg'), 'logo_url', 'https://x/logo.jpg'));
  reset role;
  select * into s from public.salons where id = sal;
